...
export interface GameClassPlugin {
  ...
  syncBenchmarkState?: (episode: EpisodeInstance) => void;
  evaluateTheorySubmission?: (
    episode: EpisodeInstance,
    submission: {
  ...
  requestOracleHint?: (episode: EpisodeInstance) => {
    text: string;
    penaltyTag?: string;
  };
  getPossessionTargets?: (episode: EpisodeInstance) => string[];
  getWorldTasks?: (episode: EpisodeInstance) => TaskState[];
  getScoreBreakdown?: (episode: EpisodeInstance) => ScoreBreakdown;
}
