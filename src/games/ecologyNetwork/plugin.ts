...
const syncEcologyState = (episode: ReturnType<typeof createPurifyChain>) => {
  ...
  setMainTask(episode, {
    ...
    description:
      episode.levelFamilyId === "pollution_vs_growth"
        ? "Balance opposed hidden ecological forces until the heart canopy survives."
        : "Purify the chain strongly enough for the distant life grove to recover.",
...
export const ecologyNetworkPlugin: GameClassPlugin = {
  ...
  syncBenchmarkState: (episode) => syncEcologyState(episode as ReturnType<typeof createPurifyChain>),
  evaluateTheorySubmission: (_episode, submission) => {
    ...
    return {
      ...
      feedback:
        mentionsEcology && mentionsInteraction
          ? "Theory accepted: hidden ecological links balance pollution, purification, and growth across worlds."
          : "Theory noted, but it does not yet explain the hidden ecology interaction rule.",
    };
  },
};
