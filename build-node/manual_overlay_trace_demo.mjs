...
var receiver = (id, worldId, x, y, interactive = false) => ({
  id: asEntityId(id),
  kind: "receiver",
  worldId: asWorldId(worldId),
  position: { x, y },
...
var syncLogicState = (episode) => {
  if (episode.levelFamilyId === "overlay_logic_stack") {
    ...
    const nextObjective = alphaOnlyAnomaly && !discoveredAlpha ? "An alpha-side anomaly is visible. Inspect the main receiver to trace where the mismatch is coming from." : alphaOnlyAnomaly && discoveredAlpha && !inspectedAlpha ? "The alpha-side chamber has been traced. Possess it and inspect the hidden device for clues." : betaOnlyAnomaly && !discoveredBeta ? "A beta-side anomaly is visible. Inspect the main receiver to trace the new mismatch into its hidden chamber." : betaOnlyAnomaly && discoveredBeta && !inspectedBeta ? "The beta-side chamber has been traced. Possess it and inspect the hidden device for clues." : !alphaActive ? "Toggle the alpha source and compare the expected OR output with the observed receiver output." : !betaActive ? "Return to the main world, toggle the beta source, and compare expected versus observed output again." : !stackedStable ? "Use both visible sources together so stacked hidden overlays act on the same main receiver." : !reportAccepted ? "You have enough evidence. Submit a report that states the hidden world count and the concrete overlay rules." : "The report has been accepted.";
    setPublicStats(episode, {
  ...
  interactionRules: [
    {
      ...
      apply: ({ episode, emit, markKeyEvent, queuePropagation }) => {
        ...
        if (entity.kind === "receiver" && episode.levelFamilyId === "overlay_logic_stack" && episode.player.worldId === "w1") {
          ...
          emit({
            turn: episode.metrics.totalSteps + 1,
            text: episode.hiddenState.hiddenFlags.anomalyDetected === true ? "The receiver confirms a mismatch, but you have already traced the currently visible anomaly." : "The receiver currently matches the visible rule, so there is no anomaly to trace."
          });
          return;
        }
...
// scripts/manual_overlay_trace_demo.ts
var runCase = (title, actions) => {
